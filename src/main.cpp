...
void PlotGFA() {
  if (f_gfa.use_count() == 0) {
    f_gfa = matplot::figure(true);
    f_gfa->width(f->width() * 3);
    f_gfa->height(f->height() * 2.5);
    f_gfa->x_position(72);
    f_gfa->y_position(760);
    f_gfa->size(960, 380);
  }
  ...
  while (1) {
    if (update_hist) {
      ...
      for (int i = 0; i < ImGuiState::DatasetTesting::eig_srv.request.gfa.size(); i++)
      {
      ...
      //std::for_each(ImGuiState::DatasetTesting::eig_srv.gfa.begin(), ImGuiState::DatasetTesting::eig_srv.gfa.end(),
      //    [idx = 0](const double &f) {
      //      ref.push_back()
      //      std::transform(ImGuiState::DatasetTesting::eig_srv.request.r_eigs.begin
      //      idx++;
      //    }
  ...
  void BackgroundVizThread() {
    ...
    while (!viewer->wasStopped()) {
      ...
      if (update_cloud) {
        ...
        // Add Label and ID
        viewer->addText(ImGuiState::DatasetTesting::cloud_id1, 0, 0, 25, 1, 1,
                        1, "text1", v0);
        viewer->addText(ImGuiState::DatasetTesting::cloud_id2, 0, 0, 25, 1, 1,
                        1, "text2", v1);
        // Add radii
  ...
  void datasetTestingPipeline(std::shared_ptr<Pipeline> & pl) {
    ...
    ImGui::SameLine();
    ImGui::RadioButton("Don't Step", &ImGuiState::DatasetTesting::should_step,
                       1);
